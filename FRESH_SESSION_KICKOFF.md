...
- **P2 (in progress)** — creative engine. Remaining: manifest→`TemplateContext` assembly (Brand tokens + copy + cached L1/L2 artifact); **copy step L0 via the free Gemini API (TEXT only)**; **image adapters via browser automation of the operator's PRO ChatGPT / AI-Studio (Nano Banana) — the free Gemini API does NOT do free images**; behind the swappable adapter; reference-research + fit-critic; brand-QA critic (code checks now, vision later); A/B + preference capture. **GATE:** an end-to-end creative — brief+pillar+topic → AI copy (human-approved) → AI imagery (browser) → composited PNG that passes the brand-QA code checks (contrast, safe-zones, dims, logo) — on a real client, human approves with ≤1 nudge; pytest green.
...
- **Resumable by design:** each phase ends at a passed GATE + a `HANDOFF.md` update, so if the run is cut off (limits/context), the next session resumes cleanly from the last green gate — nothing is lost. This is why "cut off midway" is not fatal.
- **Context hygiene:** run `/handoff` at each phase boundary; fan work to agents; don't let the main thread accumulate everything. This prevents mid-run context blowup more than the model choice does.
- **`/loop`:** start the build with `/loop` so it self-paces across a long period (there is no `/goal` command — the phase acceptance GATES above ARE the goals the loop checks itself against).
