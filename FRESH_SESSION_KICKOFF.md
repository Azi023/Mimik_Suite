...
## ⚠️ Paid-credit budget — HARD limit (READ THIS FIRST)
Image budget is ~$3 OpenAI + ~$4.89 OpenRouter. **There is NO free image-generation path** — ChatGPT browser (Cloudflare) and AI-Studio browser both fail, and the free Gemini image API returns 429 (no free image quota). All three confirmed dead.
- **Text/copy IS free** — Gemini `gemini-flash-latest`. Use freely for the L0 copy step.
- **Dev, testing, iteration, and the P2 gate → render with PLACEHOLDER solid-color backgrounds** (`TemplateContext(image_ref=None, ...)` → the compositor produces clean brand-ground creatives with no imagery, FREE). **Do NOT call any paid image API to build or test code.**
- **Real AI imagery (paid `gpt-image-1` / OpenRouter) ONLY when producing an actual, operator-approved client deliverable** — and sparingly (A/B base only). ~$8 total ≈ a few hundred images: plenty for real work, gone in an hour if wasted on a dev loop.
- Build the image adapters and **unit-test them with MOCKED responses (no network spend)**. Trigger a REAL paid generation only with explicit operator go-ahead.
- The spend-minimizing pipeline (A/B base only, generate-after-approval, cache-reuse) is mandatory — it's what keeps you inside the budget.
