"""Google Drive archive backend — the production destination for approved creatives.

A Google **service account** authenticates via the OAuth2 JWT-bearer flow (RS256-signed
assertion → access token), then the Drive v3 API stores the file under
`Mimik Clients/<Client>/<YYYY-MM>/<job>/<file>` — the same canonical tree as the local
backend, so switching backends is a config change, not a rewrite.

No google-api-python-client dependency: every network call goes through a small module-level
seam (`_post_form`, `_api_get`, `_api_post_json`, `_upload`) built on stdlib urllib and
wrapped in `asyncio.to_thread`. Tests monkeypatch those seams, so nothing here ever hits the
network in dev/test. The service-account private key only ever signs the JWT assertion; it —
and the resulting access token — are never logged.
"""
...
class GoogleDriveArchive(ArchiveBackend):
    name = "google_drive"
    ...
    async def _get_access_token(self) -> str:
        """Return a cached access token, refreshing via the JWT-bearer exchange when expired."""
