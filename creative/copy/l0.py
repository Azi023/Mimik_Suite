...
def _build_prompt(
    brand: Brand, *, pillar_name: str, topic: str, format_key: str, language: str
) -> str:
    slots = {
...
def draft_copy(
    ...
    *,
    ...
    `generate` is injectable for tests; the default is the free Gemini TEXT wrapper with
    the model pinned up front so provenance (`source_model`) matches the call. Raises
    ...
    prompt = _build_prompt(
        brand, pillar_name=pillar_name, topic=topic, format_key=format_key, language=language
    )
